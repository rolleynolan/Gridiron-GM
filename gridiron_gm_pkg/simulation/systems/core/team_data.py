    ...
    for team in teams:
        ...
        for pos, min_count in min_positions.items():
            for i in range(min_count):
                ...
                player_count += 1
        if VERBOSE_SIM_OUTPUT:
            print(f"[DEBUG] Finished minimums for {abbr}. Current roster size: {len(team.roster)}")
        ...
        while len(team.roster) < roster_size and fill_attempts < max_attempts:
            pos = random.choice(all_positions)
            idx = sum(1 for p in team.roster if getattr(p, "position", None) == pos) + 1
            ...
            player_count += 1
            fill_attempts += 1
            if len(team.roster) % 5 == 0 or len(team.roster) == roster_size:
                if VERBOSE_SIM_OUTPUT:
                    print(f"[DEBUG] {abbr} roster length: {len(team.roster)}")
        if fill_attempts >= max_attempts:
            print(f"[ERROR] Roster fill for {abbr} hit max attempts! Current size: {len(team.roster)}")
