...
def _relevant_attributes(position: str) -> list[str]:
    ...
    position_specific = Player.init_position_attributes(dummy)
    ordered = list(core.keys()) + list(position_specific.keys())
    seen = set()
    return [attr for attr in ordered if not (attr in seen or seen.add(attr))]
...
def _simulate_full_career(player_id: str, position: str, years: int = 15):
    ...
    attrs = {
        ...
        for attr in _relevant_attributes(position)
    }
    caps = {
